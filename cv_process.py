...
def segment_image_with_bbox(image_path, lable, bbox, output_mask='mask1.png'):
    """
    image_path: can be either a file path (str) or a numpy array (BGR image).
    output_mask: output mask file name.
    1) 用户可决定是否检测特定类别
    ...
    # if lable == "苹果":
    #     lable = "apple"
    # detections, vis_img = detect_objects(image_path, lable)
    ...
    if detections:
        ...
        # 使用bbox來獲得對應的邊框
        print("YOLO No detections - We used the orin bbox")
...
def segmet_image_clipseg(image_path, output_mask='mask1.png'):
    if isinstance(image_path, str):
        # 如果是字符串，说明是图像路径
        bgr_img = cv2.imread(image_path)
        if bgr_img is None:
    ...
    mask_fn = segmodel.BoxSegmenter()
    mask = mask_fn.predict_mask(image)#, prompt=prompt)
    mask = mask.squeeze().cpu().numpy()
    mask_u8 = (mask.astype(np.uint8) * 255)  # bool -> 0/255
    # 6) 保存 mask
    if mask is not None:
        
        cv2.imwrite(output_mask, mask_u8, [cv2.IMWRITE_PNG_BILEVEL, 1])
    # print(f"Segmentation saved to {output_mask}")
    else:
        print("[WARNING] Could not generate mask")
...
if __name__ == '__main__':
    # seg_mask = segment_image('detection_visualization.jpg')
    # print("Segmentation result mask shape:", seg_mask.shape if seg_mask is not None else None)
    mask = segmet_image_clipseg('detection_visualization.jpg')
    print("Segmentation result mask shape:", mask.shape if mask is not None else None)
