...
from utils import point_cloud_utils,box_filter

from cv_process import segment_image,segmet_image_clipseg
...
#---------------------------------------
    ...
    NUM_POINT = 20000 # 10000或5000
    # 如果点数足够，进行滤波处理
    if len(cloud_masked) >= NUM_POINT:
        idxs = np.random.choice(len(cloud_masked), NUM_POINT, replace=False)
        cloud_sampled = cloud_masked[idxs]
        color_sampled = color_masked[idxs]
        # cloud_sampled, color_sampled = point_cloud_utils.sample_cloud(cloud_masked, color_masked, 20000) # 这一步仿真不好用
...
def generate_grasps(end_points, cloud, visual=False):
    ...
    # 箱子碰撞检测
    T_cam_base = np.eye(4) # 相机到机器人基座的变换
    obb_filter = box_filter.BoxOBBFilter(K=K, T_cam_base=T_cam_base,    # 相机内外参
                        finger_t=0.01, finger_len=0.06,
                        expand_delta=0.001, rim_band=0.02,wall_thickness=0.10)
    obb_filter.build(end_points['point_clouds'].squeeze(0).cpu().numpy())   
    _,gg = obb_filter.filter_grasps(gg, approach_dist=0.05,pts_cam=end_points['point_clouds'].squeeze(0).cpu().numpy())
    ...
    # 4. NMS 去重 + 按置信度/得分排序（降序）
    sort_gg = gg.nms().sort_by_score()

    # 5. 返回抓取得分最高的抓取（对抓取预测的接近方向进行垂直角度限制）
    # 将 gg 转换为普通列表
    all_grasps = list(sort_gg)
...
def execute_grasp(env, gg):
    ...
    time1 = 1
    q0 = robot.get_joint()
    q1 = np.array([0.0, 0.0, np.pi / 2, 0.0, -np.pi / 2, 0.0]) # 6个关节角度,大概是水平位姿
...
if __name__ == '__main__':
    ...
    for _ in range(n): 
        ...
        mask_img_path = segmet_image_clipseg(color_img_path)
